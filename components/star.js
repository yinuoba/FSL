 ...
 *
 * @description 星级评分插件。设置不同的评分变化，星的状态发生变化，根据评分的高低，判断是哭脸还是笑脸，并可在设置评分后执行回调函数；也可通过参数传入评分并禁用鼠标时间，仅展示一个评分。
 */
FS.extend(FS, function() {
	...
	Star.main = function(options) {
		...
		// 如果允许鼠标交互，则定义如下参数
		if(self.mouseAble) {
			self.enterFun = options.enterFun || function() {};
			self.leaveFun = options.leaveFun || function() {}
		}
		self.setScoreFun = options.setScoreFun || function() {};
		// 根据parentObj计算出星节点数组
