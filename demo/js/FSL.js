...
var FS = (function (window) {
    ...
    FS.$$ = function(selector, limitNode) {
        ...
        if (typeof selector === 'string') {
            ...
            if (limitNode != null && !match_id && selector !== '*') {
                var eleLength = elements.length - 1;
                for (; eleLength >= 0; eleLength--) {
                    // 如果limitNodes数组中包含匹配出来的elements,则插入elementsLimit数组
                    if (limitNodes.contains(elements[eleLength])) {
                        elementsLimit.push(elements[eleLength]);
                    }
                }
    ...
    /**
     * @description 抛出异常,方便调试的时候查找出错误信息
    ...
    /**
     * @description 在chrome和Firefox浏览器的控制台输出错误日志
     * @param  {String} msg 日志信息
     * @return {Boolean}
     */

    FS.consoleError = function(msg) {
        if (FS.isChrome() || FS.isFirefox()) {
            console.error(msg);
            return;
        }
        return;
    ...
    FS.getChildNodes = function(node) {
        try {
            ...
            FS.error('调用getChildNodes方法出现异常：' + msg);
        }
    }
    ...
    /**
     * @ignore
     * @description 在元素上存放数据
     * @param {Element} node 节点元素
     * @param {String} key 队列名
     * @param {String} value
     */

    FS.data = function(node, key, value) {
        if (FS.isDefined(value)) {
            var map = 0;
            if (node !== window) map = node.FSMap = node.FSMap || ++uniqueId;

            storage[map] = storage[map] || {};
            storage[map].FSData = storage[map].FSData || {};
            storage[map].FSData[key] = value;
        }
        return storage && storage[node.FSMap] && storage[node.FSMap].FSData && storage[node.FSMap].FSData[key] ? storage[node.FSMap].FSData[key] : null;
    }
    ...
    FS.extend(Array.prototype, function () {
        ...
        /**
         * @description 返回指向第一个匹配元素的队列(将是一个函数数组)
         * @param key 队列名
         * @param value
         */
        ...
        function queue(key, value) {
            if (FS.isDefined(value)) {
                ...
                FS.data(this[i], key, value);
                return this;
            }
            return this[0] ? FS.data(this[0], key) : null;
        };

        /**
         * @description 从队列最前端移除一个队列函数，并执行他
         * @param key 队列名
         */
        ...
        function dequeue(key) {
            if (FS.isValue(key)) {
                for (var i = 0; i < this.length; i++) {
                    try {
                        storage[this[i].FSMap].FSData[key] = null;
                    } catch (e) {}
                }
            }
            return this;
    ...
    FS.nodesAddEvent = function(nodes, type, listener) {
        try {
            ...
            FS.error('nodesAddEvent出现异常：' + msg);
        }
    }
    ...
    FS.selectorAddEvent = function(selectors, type, listener) {
        try {
            ...
            FS.error('selectorAddEvent方法出现异常' + msg);
        }
    }

    /**
     * @description 动态创建script
     * @param {String} src script中的src
     * @example FS.createScript('http://web01.optimix.asia/events/opxLoader.js');
     */

    FS.createScript = function createScript(src) {
        var head = FS('head')[0];
     ...
     * @param [options.id] : iframe的id,
     * @param [options.src=''] : iframe中的src 默认为'',
     * @param [options.scrolling='no'] : iframe是否有滚动条，默认为no,
     * @param [options.frameborder='0'] : iframe的border，默认为0，
     * @param [options.width='0'] : 默认为'0',
     * @param [options.height='0'] : 默认为'0',
     * @param [options.zIndex='1'] : 默认为'1'
     * @example
     * FS.createIframe({
     *     id : 'iframe'+maskId,
     *     width : '100%',
     ...
     * 如不需要缓存ajax请求，可自行用FS.now()方法取得时间戳，加在url后面。
     * @param {Object} options 参数对象
    ...
    FS.ajax = function(options) {
        try {
            ...
            // 加入url是带参数的，则对url中参数的名值对编码，不然GET请求会报异常
            if (url.indexOf('?') !== -1) {
    ...
    FS.setCss = function(selector, prop) {
        try {
            ...
            FS.consoleError('setCss方法出现异常，可能是' + selector + '中有节点为定义。');
        }
    }

    /**
     * @description 隐藏某节点,需隐藏多个节点的话用逗号分割，同时可以隐藏数组中的所有节点
     * @param {Element|NodeList|Array} node 需要隐藏的节点，可以一下隐藏多个节点
     ...
     * @param {Element|NodeList|Array} node 需要显示的节点，可以一下显示多个节点，多个节点用逗号隔开
    ...
    FS.fadeIn = function(node, start, end, speed) {
        ...
        var fadeIn = setTimeout(function () {
            ...
            if (curTime < endTime) {
                fadeIn = setTimeout(arguments.callee, 13);
            } else {
                FS.opacity(node, end);
                clearTimeout(fadeIn);
                return;
    ...
    FS.fadeOut = function(node, start, end, speed) {
        ...
        var fadeOut = setTimeout(function () {
            ...
            if (curTime < endTime) {
                fadeOut = setTimeout(arguments.callee, 13);
            } else {
                FS.opacity(node, end);
    ...
    FS.slide = function(node, attr, start, end, speed) {
        ...
        // setInterval的时间间隔为speed/1000ms，如是1000ms，则每1ms执行一次
        // 用了easeOut算法
        var slideId = setTimeout(function () {
            // curTime 当前时间
            // elapsed 当前时间与开始时间之差
            // curVal 计算当前帧的值
            var curTime = FS.now(),
            ...
            if (curTime < endTime) {
                var slideId = setTimeout(arguments.callee, 13);
            } else {
                switch (attr) {
                case 'width':
                    FS.setCss(node, {
    ...
    FS.createSelectIframe = function () {
        ...
        if (FS.isIE6() && FS('select').length > 0) {
            ...
            if (FS('#iframeSelect')[0] === null) {
                ...
                // 将iframe的透明度设为0
                FS.opacity(FS('#iframeSelect')[0], 0);
            }
            FS.show(FS('#iframeSelect')[0]);
        }
    }
